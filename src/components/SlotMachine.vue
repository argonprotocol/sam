<template>
  <div class="relative border border-slate-300">
    <svg class="pointer-events-none w-full h-full relative left-[-1px] cursor-default" style="width: calc(100% + 2px)" xmlns="http://www.w3.org/2000/svg">
      <defs>
      ...
      <text class="font-bold opacity-50" x="50%" y="50%" transform="translate(0 -27)" text-anchor="middle" font-size="50" :textLength="calculateTextLength(formatAsBillions(tomorrow))" lengthAdjust="spacing">
        <tspan class="w-9" :fill="`url(#textGradientA1-${id})`">{{ id === 'supply' ? '₳' : '$' }}</tspan>
        <tspan class="w-9" :fill="`url(#textGradientA1-${id})`" v-for="x of formatAsBillions(tomorrow).split('')">{{x}}</tspan>
      </text>
      
      <text class="pointer-events-none font-bold" x="50%" y="50%" transform="translate(0 20)" text-anchor="middle" font-size="50" :textLength="calculateTextLength(formatAsBillions(today))" lengthAdjust="spacing">
        <tspan class="w-9" :fill="bgColor">{{ id === 'supply' ? '₳' : '$' }}</tspan>
        <tspan class="w-9" :fill="bgColor" v-for="x of formatAsBillions(today).split('')">{{x}}</tspan>
      </text>
      
      <text class="font-bold opacity-50" x="50%" y="50%" transform="translate(0 66)" text-anchor="middle" font-size="50" :textLength="calculateTextLength(formatAsBillions(yesterday))" lengthAdjust="spacing">
        <tspan class="w-9" :fill="`url(#textGradientA2-${id})`">{{ id === 'supply' ? '₳' : '$' }}</tspan>
